# General Mathematics (needs revision)
...
|-------------|-------------|
| [`schwartzToL2`](../OSforGFF/FunctionalAnalysis.lean#L256) | $\mathcal{S}(\mathbb{R}^d,\mathbb{C}) \hookrightarrow L^2$ as CLM |
| [`schwartzToL2'`](../OSforGFF/FunctionalAnalysis.lean#L262) | Variant for `EuclideanSpace` |
| [`linfty_mul_L2_CLM`](../OSforGFF/FunctionalAnalysis.lean#L307) | $L^\infty$ multiplier as CLM on $L^2$ |
| [`linfty_mul_L2_CLM_norm_bound`](../OSforGFF/FunctionalAnalysis.lean#L351) | $\|gf\|_2 \le C\|f\|_2$ |
| [`locallyIntegrable_of_rpow_decay_real`](../OSforGFF/FunctionalAnalysis.lean#L517) | $\|f(x)\| \le C\|x\|^{-\alpha},\;\alpha < d \implies$ locally integrable ($d \ge 3$) |
| [`integrableOn_ball_of_rpow_decay`](../OSforGFF/FunctionalAnalysis.lean#L412) | Integrability on balls for power-law singularity |
| [`schwartz_bilinear_integrable_of_translationInvariant_L1`](../OSforGFF/FunctionalAnalysis.lean#L589) | $f(x)\,K_0(x-y)\,g(y)$ integrable when $K_0 \in L^1$ |
| [`schwartz_integrable_decay`](../OSforGFF/FunctionalAnalysis.lean#L891) | $\|f(x)\| \le C(1+\|x\|)^{-N}$ for all $N$ |
| [`schwartz_vanishing_linear_bound_general`](../OSforGFF/FunctionalAnalysis.lean#L762) | $f\!\mid_{t\le 0}=0 \implies \|f(t,x)\| \le Ct$ |
| [`SchwartzMap.translate`](../OSforGFF/FunctionalAnalysis.lean#L862) | Translation of Schwartz functions |
| [`double_mollifier_convergence`](../OSforGFF/FunctionalAnalysis.lean#L1049) | Double convolution with bump functions $\to$ kernel value |
| [`polynomial_decay_integrable_3d`](../OSforGFF/FunctionalAnalysis.lean#L557) | $(1+\|x\|)^{-4}$ integrable in $\mathbb{R}^3$ |
...
| Declaration | Description |
|-------------|-------------|
| [`fourier_exponential_decay`](../OSforGFF/FourierTransforms.lean#L482) | $\widehat{e^{-\mu\|x\|}} = 2\mu/(k^2+\mu^2)$ |
| [`fourier_exp_decay_positive_halfline`](../OSforGFF/FourierTransforms.lean#L329) | $\int_0^\infty e^{ikx}\,e^{-\mu x}\,dx$ |
| [`fourier_exp_decay_negative_halfline`](../OSforGFF/FourierTransforms.lean#L387) | $\int_{-\infty}^0 e^{ikx}\,e^{\mu x}\,dx$ |
| [`fourier_inversion_exp_decay`](../OSforGFF/FourierTransforms.lean#L631) | $(2\pi)^{-1}\int e^{ikx}\,2\mu/(k^2+\mu^2)\,dk$ |
| [`fourier_lorentzian_1d`](../OSforGFF/FourierTransforms.lean#L713) | $\int e^{ikx}/(k^2+\mu^2)\,dk = (\pi/\mu)\,e^{-\mu\|x\|}$ |
| [`integrable_exponential_decay`](../OSforGFF/FourierTransforms.lean#L139) | $e^{-\mu\|x\|} \in L^1(\mathbb{R})$ |
| [`integrable_exponential_decay_fourier`](../OSforGFF/FourierTransforms.lean#L162) | $e^{ikx}\,e^{-\mu\|x\|} \in L^1(\mathbb{R})$ |
| [`tendsto_cexp_atTop_zero`](../OSforGFF/FourierTransforms.lean#L225) | $e^{cx} \to 0$ as $x \to +\infty$ when $\operatorname{Re}(c) < 0$ |
| [`tendsto_cexp_atBot_zero`](../OSforGFF/FourierTransforms.lean#L240) | $e^{cx} \to 0$ as $x \to -\infty$ when $\operatorname{Re}(c) > 0$ |
...
| [`L2_variance_time_average_bound`](../OSforGFF/L2TimeIntegral.lean#L725) | $\operatorname{Var}(\text{time avg}) \le \iint \operatorname{Cov}$ |
...
## 6. Nuclear Space Infrastructure

### Mathematical Background

The `NuclearSpace` typeclass and the proof that Schwartz space is nuclear are provided by the [gaussian-field](https://github.com/mdouglas/gaussian-field) library dependency. `NuclearSpace` uses the Pietsch characterization (seminorm nuclear dominance), and nuclearity of Schwartz space is proved via the Dynin-Mityagin theorem using the Hermite function basis.

### Key Declarations (gaussian-field library)

| Declaration | Description |
|-------------|-------------|
| `GaussianField.NuclearSpace` | Pietsch characterization typeclass for nuclear spaces |
| `DyninMityaginSpace.toNuclearSpace` | Dynin-Mityagin implies Pietsch nuclearity |
