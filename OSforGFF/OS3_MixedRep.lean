...
/-!
# OS3 Schwinger Pathway Theorems

This file contains the main theorems for converting the Schwinger representation
to the momentum representation used in the reflection positivity proof.

## Main Results

* `heatKernel_bilinear_fourier_form` : Heat kernel bilinear form in Fourier space
* `schwinger_fubini_swap` : Fubini swap for Schwinger representation
* `bessel_bilinear_eq_mixed_representation` : Bessel = mixed momentum/position form
* `bilinear_to_k0_inside` : Convert to k₀-inside form for factorization

## Dependencies

Imports `OS3_foundations_infra` for definitions and Fubini axioms.
-/
