# OS3: Reflection Positivity (needs revision)
...
|-------------|-------------|
| [`heatKernel_eq_gaussianFT`](../OSforGFF/OS3_MixedRepInfra.lean#L188) | $H_t(z) = \mathcal{F}[e^{-t\|k\|^2}](z)$ |
| [`schwinger_bilinear_integrable`](../OSforGFF/OS3_MixedRep.lean#L831) | $\int_0^\infty e^{-sm^2} \|\langle f, H_s g\rangle\|\,ds < \infty$ |
| [`schwinger_fubini_core`](../OSforGFF/OS3_MixedRep.lean#L1050) | Fubini swap: $(x,y)$-integral vs $s$-integral |
| [`schwinger_fubini_swap`](../OSforGFF/OS3_MixedRep.lean#L1130) | Pull $s$-integral inside the bilinear form |
| [`bilinear_schwinger_eq_heatKernel`](../OSforGFF/OS3_MixedRep.lean#L1269) | $\langle\Theta f, Cf\rangle = \int_0^\infty e^{-sm^2}\langle\Theta f, H_s f\rangle\,ds$ |
| [`heatKernel_bilinear_fourier_form`](../OSforGFF/OS3_MixedRep.lean#L207) | $\langle f, H_s g\rangle = \int e^{-s\|k\|^2}\hat{f}(-k)\hat{g}(k)\,dk$ |
| [`laplace_s_integral_with_norm`](../OSforGFF/OS3_MixedRep.lean#L671) | $\int_0^\infty \sqrt{\pi/s}\,e^{-t^2/(4s)-s\omega^2}\,ds = (\pi/\omega)e^{-\omega\lvert t\rvert}$ |
| [`bessel_bilinear_eq_mixed_representation`](../OSforGFF/OS3_MixedRep.lean#L1659) | Mixed (energy-momentum) representation of $\langle\Theta f, Cf\rangle$ |
| [`bilinear_to_k0_inside`](../OSforGFF/OS3_MixedRep.lean#L1721) | Rewrite with $k_0$ integral internalized |
| [`fubini_s_ksp_swap`](../OSforGFF/OS3_MixedRepInfra.lean#L2845) | Fubini swap: proper-time $s$ vs spatial momentum $\vec{k}$ |
| [`fubini_s_xy_swap`](../OSforGFF/OS3_MixedRepInfra.lean#L3061) | Fubini swap: proper-time $s$ vs spacetime $(x,y)$ |
| [`fubini_ksp_xy_swap`](../OSforGFF/OS3_MixedRepInfra.lean#L3544) | Fubini swap: spatial momentum $\vec{k}$ vs spacetime $(x,y)$ |

### Covariance RP direct proof (`OS3_CovarianceRP.lean`)

| Declaration | Description |
|-------------|-------------|
| `BlueprintAlt.rpInnerProduct` | RP inner product: $\langle \Theta f, f\rangle_C$ via Bessel kernel |
| `BlueprintAlt.weightedLaplaceFourier` | $F_\omega(\vec{k}) = \int f(x)\,e^{-\omega x_0}e^{-i\vec{k}\cdot\vec{x}}\,dx$ |
| `BlueprintAlt.mixed_representation` | Mixed representation of the RP inner product |
| `BlueprintAlt.factorization_to_squared_norm_direct` | Positive-time factorization into $\lvert F_\omega\rvert^2$ |
| `BlueprintAlt.rp_equals_squared_norm_integral` | $\langle\Theta f, f\rangle_C = \frac{1}{2(2\pi)^3}\int \frac{\lvert F_\omega(\vec{k})\rvert^2}{\omega}\,d\vec{k}$ |
| `BlueprintAlt.freeCovariance_reflection_positive_direct` | $\mathrm{Re}\langle \Theta f, f\rangle_C \geq 0$ |

### Bridge and real formulation (`OS3_ReflectionPositivity.lean`)

| Declaration | Description |
|-------------|-------------|
| `QFT.rpInnerProduct` | RP inner product in QFT namespace |
| `QFT.rpInnerProduct_eq_blueprintAlt` | Bridge: QFT and BlueprintAlt definitions agree (by `rfl`) |
| `QFT.freeCovariance_reflection_positive_bilinear` | $\mathrm{Re}\langle\Theta f, Cf\rangle \geq 0$ for complex $f \in \mathcal{S}_+$ |
| `QFT.freeCovariance_reflection_positive_bilinear_real` | $\langle\Theta f, Cf\rangle \geq 0$ for real $f \in \mathcal{S}_+$ |
| `QFT.freeCovariance_reflection_positive_real` | Alias for the real version |
...
Level 3: Single-function RP
  freeCovariance_reflection_positive_direct (BlueprintAlt)
