...
elsewhere:
- OS0 is proved in `OSforGFF/OS0_GFF.lean` via the explicit characteristic functional
  formula Z[f] = exp(½ ∑ₙ λₙ(f)²)
...
- OS0 is supplied by `gaussianFreeField_satisfies_OS0` via the explicit CF formula
...
theorem gaussianFreeField_satisfies_all_OS_axioms (m : ℝ) [Fact (0 < m)] :
  ...
  -- OS0 from the explicit characteristic functional formula
  os0 := gaussianFreeField_satisfies_OS0 m
