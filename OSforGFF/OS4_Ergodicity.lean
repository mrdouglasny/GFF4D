...
/-! ## Helper Lemmas for Time Translation

The core time translation lemmas (decompose_fst/snd, timeTranslationDistribution_pairingℂ,
continuous_distributionPairingℂ_timeTranslation) are now in OS4_MGF.lean.
-/
...
lemma gff_exp_time_translated_memLp_two (m : ℝ) [Fact (0 < m)] (s : ℝ) (f : TestFunctionℂ) :
  ...
  have h_dom : Integrable (fun ω : FieldConfiguration =>
    ...
    · convert h_L2 using 1
      ...
      ring_nf
    · exact (Real.continuous_exp.measurable.comp (continuous_abs.measurable.comp (fieldConfiguration_eval_measurable _))).aestronglyMeasurable
...
The core MGF and invariance lemmas (gff_mgf_formula, freeCovarianceℂ_bilinear_timeTranslation_invariant,
gff_generating_time_invariant) are now in OS4_MGF.lean.
-/
...
The gff_joint_mgf_factorization lemma is now in OS4_MGF.lean.
-/

/-! ## GFF Covariance Continuity (proved, no axiom) -/
