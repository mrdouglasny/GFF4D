# Basic Definitions (needs revision)
...
|-------------|-------------|
| [`STDimension`](../OSforGFF/Basic.lean#L113) | Spacetime dimension ($= 4$) |
| [`SpaceTime`](../OSforGFF/Basic.lean#L114) | $\mathbb{R}^4 =$ `EuclideanSpace ℝ (Fin 4)` |
| [`getTimeComponent`](../OSforGFF/Basic.lean#L118) | $x \mapsto x_0$ |
...
| `QFT.E` | Euclidean group $E(4) = O(4) \rtimes \mathbb{R}^4$ |
| `QFT.act` | Group action of $E(4)$ on spacetime |
...
| Declaration | Description |
|-------------|-------------|
| `QFT.timeReflection` | $\Theta: (t,\mathbf{x}) \mapsto (-t,\mathbf{x})$ |
| `QFT.timeReflectionMatrix` | Matrix representation of $\Theta$ |
| `QFT.timeReflectionLE` | $\Theta$ as a linear isometric equivalence |
| `QFT.timeReflection_measurePreserving` | $\Theta$ preserves Lebesgue measure |
| `QFT.compTimeReflection` | $f \mapsto f \circ \Theta$ on $\mathcal{S}(\mathbb{R}^4,\mathbb{C})$ |
| `QFT.compTimeReflectionReal` | $f \mapsto f \circ \Theta$ on $\mathcal{S}(\mathbb{R}^4,\mathbb{R})$ |

### Key Declarations (`OSforGFF/QFTHilbertSpace.lean`)

| Declaration | Description |
|-------------|-------------|
| `QFTHilbertSpace.SpatialCoords` | Spatial coordinates $\mathbb{R}^3$ |
| `QFTHilbertSpace.spatialPart` | Spatial projection $\mathbb{R}^4 \to \mathbb{R}^3$ |
| `QFTHilbertSpace.E` | Relativistic energy $E(m,k) = \sqrt{\|k\|^2 + m^2}$ |
...
- **TestFunctionC:** $\mathcal{S}(\mathbb{R}^4, \mathbb{C})$
...
| Declaration | Description |
|-------------|-------------|
| [`TestFunction`](../OSforGFF/Basic.lean#L135) | $\mathcal{S}(\mathbb{R}^4, \mathbb{R})$ |
| `TestFunctionC` | $\mathcal{S}(\mathbb{R}^4, \mathbb{C})$ |
| [`FieldConfiguration`](../OSforGFF/Basic.lean#L168) | $\mathrm{WeakDual}\;\mathbb{R}\;\mathrm{TestFunction}$ |
| [`distributionPairing`](../OSforGFF/Basic.lean#L199) | $\langle\omega, f\rangle$ for $\omega :$ FieldConfiguration, $f :$ TestFunction |
| [`distributionPairingCLM`](../OSforGFF/Basic.lean#L214) | The pairing as CLM: $\mathrm{TestFunction} \to \mathrm{FieldConfiguration} \to_L \mathbb{R}$ |
| `distributionPairingC_real` | $\langle\omega, f\rangle_{\mathbb{C}}$ for $f : \mathcal{S}(\mathbb{R}^4,\mathbb{C})$ |
| [`complex_testfunction_decompose`](../OSforGFF/Basic.lean#L283) | $f = \operatorname{Re}(f) + i\,\operatorname{Im}(f)$ |
...
| `timeTranslationSchwartzC` | $T_s$ on $\mathcal{S}(\mathbb{R}^4,\mathbb{C})$ |
...
| Declaration | Description |
|-------------|-------------|
| [`GJGeneratingFunctional`](../OSforGFF/Basic.lean#L240) | $Z[J] = \int e^{i\langle\omega,J\rangle}\,d\mu$ for real $J$ |
| `GJGeneratingFunctionalC` | Complex generating functional for complex $J$ |
| [`GJMean`](../OSforGFF/Basic.lean#L338) | $\int \langle\omega,f\rangle\,d\mu$ |

### Key Declarations (`OSforGFF/Schwinger.lean`)

| Declaration | Description |
|-------------|-------------|
| [`SchwingerFunction`](../OSforGFF/Schwinger.lean#L130) | $S_n(f_1,\ldots,f_n)$ |
| `SchwingerFunction1` | 1-point function (mean) |
| `SchwingerFunction2` | $S_2(f,g)$ for real test functions |
| `SchwingerFunctionC` | Complex $n$-point function |
| `SchwingerFunctionC2` | Complex $S_2(f,g)$ |
| [`CovarianceBilinear`](../OSforGFF/Schwinger.lean#L182) | All $\langle\omega,f\rangle\langle\omega,g\rangle$ integrable |
| [`IsGaussianMeasure`](../OSforGFF/Schwinger.lean#L331) | $Z[J] = e^{-\frac{1}{2}S_2(J,J)}$ |
