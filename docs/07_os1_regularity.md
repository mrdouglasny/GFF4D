...
1. Use $|e^z| = e^{\operatorname{Re}(z)}$ to extract the norm.
2. Decompose $f = f_{\mathrm{re}} + if_{\mathrm{im}}$ and expand the bilinear form.
3. Use positive-definiteness of the covariance to bound $-\operatorname{Re}\langle f, Cf\rangle \le \langle f_{\mathrm{im}}, Cf_{\mathrm{im}}\rangle$.
...
| [`gff_generating_norm_eq`](../OSforGFF/OS1_GFF.lean#L219) | $\|Z[f]\| = \exp(-\tfrac{1}{2}\operatorname{Re}\langle f, Cf\rangle)$ |
| [`gff_generating_bound_by_imaginary`](../OSforGFF/OS1_GFF.lean#L229) | $\exp(-\tfrac{1}{2}\operatorname{Re}\langle f, Cf\rangle) \le \exp(\tfrac{1}{2}\langle f_{\mathrm{im}}, Cf_{\mathrm{im}}\rangle)$ |
...
$$\|Z[f]\| = |\exp(-\tfrac{1}{2}\langle f, Cf\rangle_{\mathbb{C}})| = \exp\!\left(-\tfrac{1}{2}\operatorname{Re}\langle f, Cf\rangle_{\mathbb{C}}\right)$$

This uses the identity $|e^z| = e^{\operatorname{Re}(z)}$ and the Gaussian structure of the GFF measure via `gff_complex_generating` and `gff_two_point_equals_covarianceℂ_free`.
...
$$\operatorname{Re}\langle f, Cf\rangle = \operatorname{Re}\langle f_{\mathrm{re}}, Cf_{\mathrm{re}}\rangle - \operatorname{Re}\langle f_{\mathrm{im}}, Cf_{\mathrm{im}}\rangle$$

The cross terms vanish because the covariance is real-valued and symmetric on real test functions (`freeCovarianceℂ_bilinear_agrees_on_reals`, `freeCovarianceℂ_bilinear_symm`). Since the covariance is positive semi-definite (`freeCovarianceℂ_positive`), $\operatorname{Re}\langle f_{\mathrm{re}}, Cf_{\mathrm{re}}\rangle \ge 0$, giving:

$$-\operatorname{Re}\langle f, Cf\rangle \le \operatorname{Re}\langle f_{\mathrm{im}}, Cf_{\mathrm{im}}\rangle$$
