...
**OS-1 (Regularity):**
- Proved in `OSforGFF/OS1_GFF.lean` via Fourier/momentum space methods for the free field
...
This namespace contains an alternative proof of OS0 for Gaussian measures via
the explicit quadratic form expansion. The main proof now used by GFFmaster.lean
is in `OSforGFF/OS0_GFF.lean`.

These lemmas are kept for reference and may be removed in a future cleanup.
-/
...
Note: The main proof is in `OSforGFF/OS0_GFF.lean`.
-/
