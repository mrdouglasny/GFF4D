...
All axioms in this file are listed here for easy reference.
See `texts/axioms.txt` for full justification. -/
...
/-- The Fourier transform of the exponential decay function e^{-μ|x|} with
    negative frequency convention: ∫ e^{-ikx} e^{-μ|x|} dx = 2μ/(k² + μ²). -/
lemma fourier_exponential_decay (μ : ℝ) (hμ : 0 < μ) (k : ℝ) :
  ...
  have h_cast : ∀ x : ℝ, Complex.exp (Complex.I * (↑(-k)) * ↑x) * ↑(Real.exp (-μ * |x|)) =
      Complex.exp (Complex.I * (-↑k) * ↑x) * ↑(Real.exp (-μ * |x|)) := by
    intro x; congr 2; push_cast; ring
  have h_int_Iic : IntegrableOn
      (fun x : ℝ => Complex.exp (Complex.I * (-↑k) * ↑x) * ↑(Real.exp (-μ * |x|))) (Set.Iic 0) volume := by
    rw [show (fun x : ℝ => Complex.exp (Complex.I * (-↑k) * ↑x) * ↑(Real.exp (-μ * |x|))) =
        (fun x : ℝ => Complex.exp (Complex.I * (↑(-k)) * ↑x) * ↑(Real.exp (-μ * |x|))) from by ext x; rw [h_cast]]
    exact h_int_full.integrableOn
  have h_int_Ioi : IntegrableOn
      (fun x : ℝ => Complex.exp (Complex.I * (-↑k) * ↑x) * ↑(Real.exp (-μ * |x|))) (Set.Ioi 0) volume := by
    rw [show (fun x : ℝ => Complex.exp (Complex.I * (-↑k) * ↑x) * ↑(Real.exp (-μ * |x|))) =
        (fun x : ℝ => Complex.exp (Complex.I * (↑(-k)) * ↑x) * ↑(Real.exp (-μ * |x|))) from by ext x; rw [h_cast]]
    exact h_int_full.integrableOn
  ...
  have h_Iic : ∫ x : ℝ in Set.Iic 0, Complex.exp (Complex.I * (-k) * x) * Real.exp (-μ * |x|) =
    ...
    intro x hx; simp only [Set.mem_Iic] at hx; simp only [abs_of_nonpos hx]; ring_nf
  -- On Ioi: |x| = x
  have h_Ioi : ∫ x : ℝ in Set.Ioi 0, Complex.exp (Complex.I * (-k) * x) * Real.exp (-μ * |x|) =
      ∫ x : ℝ in Set.Ioi 0, Complex.exp (Complex.I * (-k) * x) * Real.exp (-μ * x) := by
    apply MeasureTheory.setIntegral_congr_fun measurableSet_Ioi
    intro x hx; simp only [Set.mem_Ioi] at hx; simp only [abs_of_pos hx]
  rw [h_Iic, h_Ioi]
  convert fourier_exponential_decay_split μ hμ (-k) using 2 <;> simp
...
lemma fourierIntegral_expDecayFun_eq (μ : ℝ) (hμ : 0 < μ) (ξ : ℝ) :
  ...
  have h_inner : ∀ v : ℝ, @inner ℝ ℝ _ v ξ = v * ξ := fun v => by
    simp [mul_comm]
  simp_rw [h_inner]
  -- Rewrite integral to match fourier_exponential_decay with k = 2πξ
  ...
  convert h using 1
  · congr 1; ext v; congr 1; congr 1; push_cast; ring
