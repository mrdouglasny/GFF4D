...
/-!
# OS3 Foundation Infrastructure

This file contains the foundational definitions, axioms, and helper lemmas needed
for the reflection positivity (OS3) proof.

## Contents

### Definitions
* `spatialDot` : Inner product on spatial coordinates
* `spacetimeDecomp` : Decomposition of SpaceTime into time and spatial components
* `weightedLaplaceFourier` : The weighted Laplace-Fourier transform F_ω(k)

### Axioms (3 Fubini/integrability statements)
* `schwinger_bound_integrable_fubini` : Integrability for Schwinger form
* `fubini_s_ksp_swap` : Fubini swap for s ↔ k_sp integrals
* `fubini_s_xy_swap` : Fubini swap for s ↔ (x,y) integrals

### Key Theorems
* `fubini_ksp_xy_swap` : Fubini swap for k_sp ↔ (x,y) integrals (proved)
* `laplace_integral_half_power` : Bessel K_{1/2} identity
* `heatKernel_eq_gaussianFT` : Heat kernel = Gaussian Fourier transform
-/
-- Note: FunctionalAnalysis.SchwartzLinearBound has a generic version for SchwartzMap (ℝ × E) ℂ.
-- This file has a specific version for SpaceTime = EuclideanSpace ℝ (Fin 4) which handles
-- the EuclideanSpace structure directly (boundary point construction, etc.).
...
The spacetime decomposition infrastructure (spacetimeDecomp, measure preservation,
norm decomposition) is now in `OSforGFF.SpacetimeDecomp`.

The Schwartz Tonelli factorization theorem is in `OSforGFF.SchwartzTonelli`.
